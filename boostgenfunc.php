...
function boostgen() {
	...
	if (rand(1, 8) == 8) {
		$sql = "SELECT number FROM btz WHERE rarity='M'";
		$result = $conn->query($sql);
	} else {
		$sql = "SELECT number FROM btz WHERE rarity='R'";
		$result = $conn->query($sql);
	}
		$offset = 0;
	...
	//assign uncommons
	$sql = "SELECT number FROM btz WHERE rarity='U'";
	$result = $conn->query($sql);
	$offset = 0;
	while($row = $result->fetch_assoc()) {
		$temp[$offset] = $row["number"];
	...
	if (rand(1,6) == 6) {
		...
		$booster[4] = $temp[0];
		unset($temp);
		$sql = "SELECT number FROM btz WHERE rarity='C'";
		$result = $conn->query($sql);
		while($row = $result->fetch_assoc()) {
			$temp[$offset] = $row["number"];
			++$offset;
	...
	unset($temp); //purge the array previously created as fetching a new set of cards by a different rarity somehow fails to override the previously assigned values
	//assign a basic land
	$sql = "SELECT number FROM btz WHERE rarity='B'";
	$result = $conn->query($sql);
		$offset = 0;
		while($row = $result->fetch_assoc()) {
			$temp[$offset] = $row["number"];
